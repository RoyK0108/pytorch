...
class Backend(str):  # noqa: SLOT000
    ...
    def _ensure_backend_registered(cls, name: str) -> None:
        ...
        elif normalized_name in _BUILTIN_BACKEND_REGISTRARS:
            # Builtin backend entry points are published through torch's
            # distribution metadata, which is not discoverable in every
            # environment (e.g. when torch is not pip-installed). Register the
            # builtin backend directly in that case. Third-party backends still
            # load through their entry point above.
            registrar = _BUILTIN_BACKEND_REGISTRARS[normalized_name]
        else:
            return
        ...
        if upper_name not in cls._plugins:
            raise RuntimeError(
                f"c10d backend {normalized_name} did not register a backend"
            )
...
# Direct registrars for builtin backends, used as a fallback by
# Backend._ensure_backend_registered when torch's distribution metadata (and
# thus its "torch.distributed.backends" entry points) is not discoverable.
_BUILTIN_BACKEND_REGISTRARS: dict[str, Callable[[], None]] = {
    Backend.MPI: _register_builtin_mpi_backend,
    Backend.GLOO: _register_builtin_gloo_backend,
    Backend.NCCL: _register_builtin_nccl_backend,
    Backend.UCC: _register_builtin_ucc_backend,
    Backend.XCCL: _register_builtin_xccl_backend,
}
