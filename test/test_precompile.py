...
class TestPrecompile(TestCase):
    ...
    def test_wrong_param_count_rejected_inlined(self):
        # The same guard fires on the inlined (no-cache) path: strip the artifact so
        # load() execs python_code, then call with a structurally different model.
        m = torch.nn.Linear(4, 3).eval()
        x = torch.randn(5, 4)
        code, cache = torch.precompile(lambda model, x: model(x), m, x)
        blob = torch.load(io.BytesIO(cache), weights_only=True)
        ...
        with self.assertRaisesRegex(RuntimeError, "structurally identical"):
            f_c(bigger, x)
